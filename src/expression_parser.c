...
int GetIntResult(Token *operand_1, Token *operand_2, TOKEN_TYPE operator, Symtable * symtable, bool *zero_division, bool *type_incompatibility)
{
    ...
    if (operand_1->token_type == IDENTIFIER_TOKEN)
    {
        ...
        if (symbol_1->type == DOUBLE64_TYPE)
            value_1 = (int)*(double *)(symbol_1->value);
        else
            value_1 = *(int *)(symbol_1->value);
    }

    else
        value_1 = strtol(operand_1->attribute, NULL, 10);
    ...
    if (operand_2->token_type == IDENTIFIER_TOKEN)
    {
        ...
        if (symbol_2->type == DOUBLE64_TYPE)
            value_2 = (int)*(double *)(symbol_2->value);
        else
            value_2 = *(int *)(symbol_2->value);
    }

    else
        value_2 = strtol(operand_2->attribute, NULL, 10);
...
double GetDoubleResult(Token *operand_1, Token *operand_2, TOKEN_TYPE operator, Symtable * symtable, bool *zero_division, bool *type_incompatibility)
{
    ...
    if (operand_1->token_type == IDENTIFIER_TOKEN)
    {
        // check if we don't have to convert it
        VariableSymbol *symbol_1 = FindVariableSymbol(symtable, operand_1->attribute);
        if (symbol_1->type == INT32_TYPE)
            value_1 = (double)*(int *)(symbol_1->value);
        else
            value_1 = *(double *)(symbol_1->value);
    }

    else
        value_1 = strtod(operand_1->attribute, NULL);

    if (operand_2->token_type == IDENTIFIER_TOKEN)
    {
        VariableSymbol *symbol_2 = FindVariableSymbol(symtable, operand_2->attribute);
        if (symbol_2->type == INT32_TYPE)
            value_2 = (double)*(int *)(symbol_2->value);
        else
            value_2 = *(double *)(symbol_2->value);
    }

    else
        value_2 = strtod(operand_2->attribute, NULL);
...
bool GetBooleanResult(Token *operand_1, Token *operand_2, TOKEN_TYPE operator, Symtable * symtable, bool float_expression, bool *nullable, bool *incompaible_types)
{
    ...
    if (float_expression)
    {
        ...
        if (operand_1->token_type != IDENTIFIER_TOKEN)
            double_1 = strtod(operand_1->attribute, NULL);
        else if (operand_1->token_type == INTEGER_32)
            double_1 = (double)*(int *)(FindVariableSymbol(symtable, operand_1->attribute));
        else
            double_1 = *(double *)(FindVariableSymbol(symtable, operand_1->attribute));

        // compute the second value
        if (operand_2->token_type != IDENTIFIER_TOKEN)
            double_2 = strtod(operand_2->attribute, NULL);
        else if (operand_2->token_type == INTEGER_32)
            double_2 = (double)*(int *)(FindVariableSymbol(symtable, operand_2->attribute));
        else
            double_2 = *(double *)(FindVariableSymbol(symtable, operand_2->attribute));
    }
    ...
    else
    {
        // compute the first value
        if (operand_1->token_type != IDENTIFIER_TOKEN)
            int_1 = strtol(operand_1->attribute, NULL, 10);
        else
            int_1 = *(int *)(FindVariableSymbol(symtable, operand_1->attribute));

        // compute the second value
        if (operand_2->token_type != IDENTIFIER_TOKEN)
            int_2 = strtol(operand_2->attribute, NULL, 10);
        else
            int_2 = *(int *)(FindVariableSymbol(symtable, operand_2->attribute));
    }
...
ExpressionReturn *EvaluatePostfixExpression(TokenVector *postfix, Parser parser)
{
    ...
    for (int i = 0; i < postfix->length; i++)
    {
        ...
        case DOUBLE_64:
            ...
            if (current_token->token_type == DOUBLE_64)
                float_expression = true;
            break;
        ...
        case IDENTIFIER_TOKEN: // put on top of stack
            ...
            if ((var_1 = FindVariableSymbol(parser.symtable, current_token->attribute)) == NULL)
            {
                ...
                if (!IsTokenInString(postfix, operand_1))
                    DestroyToken(operand_1);
                if (!IsTokenInString(postfix, operand_2))
                    DestroyToken(operand_2);
        ...
        case SUBSTRACTION_OPERATOR:
            ...
            if (operand_2->token_type == IDENTIFIER_TOKEN)
            {
                ...
                if (var_2->type == DOUBLE64_TYPE)
                    float_expression = true;
            }

            // update the type flag if it is needed
            if (operand_1->token_type == DOUBLE_64 || operand_2->token_type == DOUBLE_64)
                float_expression = true;
        ...
        case SEMICOLON: // end condition
            ...
            if (!IsTokenInString(postfix, result))
                DestroyToken(result);
            DestroyStackAndVector(postfix, stack);
